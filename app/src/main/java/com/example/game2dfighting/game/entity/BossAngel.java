...
//
//        // Hiệu ứng tụ lực
//        // Hiệu ứng tụ lực (vẽ vòng tròn viền đỏ rực - new)
//        if (chargingLaser) {
//            float progress = Math.min(1f, (System.currentTimeMillis() - chargeStartMs) / (float) LASER_CHARGE_TIME);
//
//            float radius = 80 + 40 * progress; // bán kính tăng dần
//            float cx = x + w / 2f - camX;
//            float cy = y + h / 2f - camY;
//
//            Paint ring = new Paint(Paint.ANTI_ALIAS_FLAG);
//            ring.setStyle(Paint.Style.STROKE);
//            ring.setStrokeWidth(10f + 6f * progress);
//            ring.setColor(0xFFFF3333); // đỏ tươi
//            ring.setShadowLayer(12f, 0, 0, 0xFFFF0000);
//            ring.setAlpha((int) (150 + 100 * Math.sin(progress * Math.PI))); // nhấp nháy nhẹ theo progress
//
//            // Vẽ vòng tròn viền chính
//            c.drawCircle(cx, cy, radius, ring);
//
//            // Vẽ thêm 1 vòng tròn nhạt hơn bên ngoài cho đẹp hơn
//            Paint outer = new Paint(ring);
//            outer.setStrokeWidth(6f);
//            outer.setAlpha(100);
//            c.drawCircle(cx, cy, radius + 20, outer);
//        }
//
//
//
 ...
 * BossAngel có 2 hình thái:
 * - Form1: Bình thường, bắn đạn xanh dương tỏa ra nhiều hướng.
 * - Form2: Cuồng nộ khi còn dưới 30% máu, bắn laser đỏ tập trung.
 */
public class BossAngel extends GameObject {
    private final Context ctx;
    ...
    private void setupPaint() {
        laserPaint.setColor(0xFFFF4444);
        laserPaint.setStrokeWidth(10f);
        laserPaint.setStyle(Paint.Style.STROKE);
        laserPaint.setShadowLayer(12f, 0, 0, 0xFFFF2222);
    }
    ...
    public void update(long dtMs, Player player, int hp, int hpMax) {
        // Kích hoạt cuồng nộ khi còn dưới 30% máu
        if (!furyMode && hp <= hpMax * 0.3f) {
            furyMode = true;
            if (idleForm2 != null) idleForm2.reset();
        }
        ...
        // Chỉ tấn công khi attackEnabled = true
        if (!attackEnabled) {
            if (furyMode) idleForm2.update(dtMs);
            else idleForm1.update(dtMs);
            return;
        ...
        // FORM 1: bắn đạn xanh
        if (!furyMode && now >= nextShootAtMs) {
            shootCircleBullets();
            nextShootAtMs = now + FORM1_SHOOT_INTERVAL;
        }

        // FORM 2: vừa bắn đạn 2 lần, vừa tụ lực bắn laser
        if (furyMode && !chargingLaser && now >= nextShootAtMs) {
            shootCircleBullets();  // đợt 1
            shootCircleBullets();  // đợt 2
            chargingLaser = true;
            chargeStartMs = now;
            nextShootAtMs = now + FORM2_LASER_INTERVAL;
        }

        // Sau 3s tụ lực thì bắn laser
        if (chargingLaser && now - chargeStartMs >= LASER_CHARGE_TIME) {
            shootLaser(player);
            chargingLaser = false;
        }
    ...
    public void draw(Canvas c, int camX, int camY, Paint p) {
        ...
        // Tụ lực: chỉ vẽ vòng tròn outline, không fill
        if (chargingLaser) {
            float progress = Math.min(1f, (System.currentTimeMillis() - chargeStartMs) / (float) LASER_CHARGE_TIME);
            Paint outline = new Paint(laserPaint);
            outline.setAlpha((int)(255 * (0.5f + 0.5f * progress)));
            outline.setStyle(Paint.Style.STROKE);
            outline.setStrokeWidth(6f + 6f * progress);
...
//hàm xly boss angle hẹo
    ...
    private void shootLaser(Player player) {
        ...
        if (len == 0) return;
        dx /= len; dy /= len;
        lasers.add(new LaserBeam(cx, cy, dx, dy, 14f, 0xFFFF0000));
    }

    public boolean isFuryMode() { return furyMode; }
    public List<BossBullet> getBullets() { return bullets; }
    public List<LaserBeam> getLasers() { return lasers; }

    // === Di chuyển đuổi theo người chơi (pursue) ===
    public void pursue(int targetX, int targetY, long dtMs) {
        if (state == State.DIE) return;
        if (chargingLaser) return; // không đuổi khi đang tụ lực

        // Boss chỉ thực hiện pursue khi đã được bật tấn công
